...
class CreditNote extends OrderDocumentMethods {
	...
	public function __construct( $order = 0 ) {
		...
		$this->title = __( 'Dobropis', 'woocommerce-pdf-invoices-packing-slips' );
	...
	public function get_title() {
		...
		$title = __( 'Dobropis', 'woocommerce-pdf-invoices-packing-slips' );
	...
	public function get_number_title() {
		...
		$title = __( 'Číslo faktúry:', 'woocommerce-pdf-invoices-packing-slips' );
	...
	public function get_date_title() {
		// override to allow for language switching!
		$title = __( 'Dátum vystavenia:', 'woocommerce-pdf-invoices-packing-slips' );
       ...
       public function get_woocommerce_totals() {
               ...
               foreach ( $totals as $key => $total ) {
                       if ( isset( $total['value'] ) && substr( trim( $total['value'] ), 0, 1 ) !== '-' ) {
                               $numeric = floatval( preg_replace( '/[^0-9.-]/', '', html_entity_decode( strip_tags( $total['value'] ) ) ) );
                               if ( 0.0 !== $numeric ) {
