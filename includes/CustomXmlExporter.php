...
class CustomXmlExporter {
    ...
    public function __construct() {
        ...
        add_filter( 'wpo_wcpdf_document_output_formats', array( $this, 'add_invoice_xml_output_format' ), 10, 2 );
        add_filter( 'wpo_wcpdf_document_is_enabled', array( $this, 'enable_invoice_xml_output' ), 10, 3 );
    ...
    /**
     * Make sure the invoice document advertises XML output alongside PDF/UBL.
     */
    public function add_invoice_xml_output_format( array $formats, $document ): array {
    ...
    /**
     * Always enable the XML output toggle for invoices.
     */
    public function enable_invoice_xml_output( $enabled, string $document_type, string $output_format ) {
    ...
    /**
     * Expose invoice XML in the bulk actions dropdown on the orders list.
     */
    public function register_invoice_xml_bulk_action( array $actions ): array {
        if ( ! isset( $actions['invoice_xml'] ) ) {
            $actions['invoice_xml'] = esc_html__( 'XML Invoice', 'woocommerce-pdf-invoices-packing-slips' );
        }
    ...
    /**
     * Stream the XML export requested through manual/bulk document generation.
     *
     * @param OrderDocument|BulkDocument $document
     * @param array                      $order_ids
     * @param array                      $request
     */
    public function handle_manual_invoice_xml_request( $document, array $order_ids, array $request ): void {
        if ( empty( $order_ids ) ) {
            wcpdf_output_error( esc_html__( 'No orders were provided for the XML export.', 'woocommerce-pdf-invoices-packing-slips' ) );
        }

        $force_regeneration = isset( $request['regenerate'] ) && wc_string_to_bool( $request['regenerate'] );

        if ( 1 === count( $order_ids ) ) {
            $order_id = absint( reset( $order_ids ) );

            if ( $document instanceof BulkDocument ) {
                $document = wcpdf_get_document( 'invoice', $order_id, true );
            }

            $order = $document instanceof OrderDocument && $document->order instanceof WC_Abstract_Order
                ? $document->order
                : wc_get_order( $order_id );

            if ( ! $document instanceof OrderDocument || ! $order instanceof WC_Abstract_Order ) {
                wcpdf_output_error( esc_html__( 'The invoice document could not be loaded for the XML export.', 'woocommerce-pdf-invoices-packing-slips' ) );
            }
            ...
            if ( empty( $path ) || ! WPO_WCPDF()->file_system->exists( $path ) ) {
                wcpdf_output_error( esc_html__( 'The XML file could not be generated.', 'woocommerce-pdf-invoices-packing-slips' ) );
            }
        ...
        foreach ( $order_ids as $order_id ) {
            ...
            $single_document = wcpdf_get_document( 'invoice', $order, true );

            if ( ! $single_document instanceof OrderDocument ) {
                continue;
            }

            $path = $this->export_invoice_xml( $single_document, $order, $force_regeneration );

            if ( empty( $path ) || ! WPO_WCPDF()->file_system->exists( $path ) ) {
                $path = $this->get_invoice_xml_path( $single_document, $order );
            }
            ...
            $data = $this->prepare_invoice_data( $single_document, $order );
        ...
        if ( empty( $entries ) ) {
            wcpdf_output_error( esc_html__( 'No XML files could be generated for the selected orders.', 'woocommerce-pdf-invoices-packing-slips' ) );
        }

        $download_name = apply_filters(
            'wpo_wcpdf_invoice_xml_bulk_filename',
            sprintf( 'invoice-xml-%s.xml', gmdate( 'Ymd-His' ) ),
            $order_ids,
            $document
        );

        $bulk_xml = $this->generate_xml_contents( $entries );
        $bulk_xml = apply_filters( 'wpo_wcpdf_invoice_xml_bulk_export_xml', $bulk_xml, $entries, $order_ids, $document );
        $bulk_xml = $this->normalize_xml_output( $bulk_xml );

        if ( '' === $bulk_xml ) {
            wcpdf_output_error( esc_html__( 'The combined XML export could not be generated.', 'woocommerce-pdf-invoices-packing-slips' ) );
        }

        $this->stream_invoice_xml_string( $bulk_xml, $download_name );
    }
    ...
    /**
     * Stream a ZIP archive containing one or more invoice XML files.
     */
    /**
     * Stream raw XML contents directly to the browser.
     */
    protected function stream_invoice_xml_string( string $xml, string $filename ): void {
        header( 'Content-Type: application/xml; charset=utf-8' );
        header( sprintf( 'Content-Disposition: attachment; filename="%s"', addcslashes( $filename, "\\\"" ) ) );
    ...
    protected function normalize_xml_output( $xml ): string {
        ...
        // Remove UTF BOM variants that may precede the declaration.
        if ( 0 === strpos( $xml, "\xEF\xBB\xBF" ) ) { // UTF-8 BOM.
            $xml = substr( $xml, 3 );
        } elseif ( 0 === strpos( $xml, "\xFE\xFF" ) || 0 === strpos( $xml, "\xFF\xFE" ) ) { // UTF-16 BOMs.
            $xml = substr( $xml, 2 );
        } elseif ( 0 === strpos( $xml, "\x00\x00\xFE\xFF" ) || 0 === strpos( $xml, "\xFF\xFE\x00\x00" ) ) { // UTF-32 BOMs.
            $xml = substr( $xml, 4 );
        }
        ...
        // Strip any leading control/separator characters (including zero-width spaces) before trimming ASCII whitespace.
        $xml = preg_replace( '/\A[\p{C}\p{Z}]*/u', '', $xml );
        ...
        // Guarantee we start at the first tag.
    ...
    /**
     * Create XML string from one or more invoice datasets.
     */
    protected function generate_xml_contents( array $data ): string {
        ...
        $is_associative = ! empty( $data ) && array_keys( $data ) !== range( 0, count( $data ) - 1 );
        $entries        = $is_associative ? array( $data ) : $data;
